...
def result(request):
    ...
    if request.method == "POST":
        ...
        question_text = request.POST.get("question_text")
        ###########################################
        true_or_false = request.POST.get("true_or_false")
        if true_or_false:
        ...
        with open('/tmp/data.csv', 'wb') as csvfile:
            ...
            writer.writeheader()
            writer.writerow({'Question Type': question_type, 'Forecast': forecast, 'Question Purpose': question_purpose, 'Question Text': question_text, '# of Choices': true_or_false, 'Category': category, 'User': user_name, 'Group': group_name, 'Assignment Name': assignment_name, 'Date Submitted': date_submitted})
            csvfile.write("\n\n\n");
        #########################RAUL CODE##############################
        ...
        if assignment_name:
            ...
            #print "Assignment Object: ", assignmentObj
        ...
        if question_type is not None:
            QSet = Question.objects.filter(id__in=QSet, question_type=question_type)
            #print 'GOT VALUE'
        if forecast is not None:
            QSet = Question.objects.filter(id__in=QSet, forecast=forecast)
            #print 'GOT VALUE'
        if question_purpose is not None:
            QSet = Question.objects.filter(id__in=QSet, corporate_training=question_purpose)
            #print 'GOT VALUE'
        if question_text:
            QSet = Question.objects.filter(id__in=QSet, question_text=question_text)
            #print 'GOT VALUE'
        if true_or_false:
            QSet = Question.objects.filter(id__in=QSet, num_of_choices=true_or_false)
            #print 'GOT VALUE'
        ...
        if user_name:
            upload_user = User.objects.get(username = user_name)#no need to change
        ...
        for a in ASet:
            #print a
            QObj = Question.objects.get(id=a.question_id.id)
            #print 'TRUE VALUE: ', QObj.true_value

        temp = ASet.values()
        with open('/tmp/data.csv', 'a') as csvfile:
            assessment_fieldnames = ['Question ID', 'Date of assessment','User ID','Operator','Answer Text','Details Of Assessment','Option Text','ID']
            ...
            for value in temp:
             ...
             'User ID':value['user_id_id'],'Operator':value['operator'],'Answer Text':value['answer_text'],
        ...
        # Fields which are written to file
        ...
        #print "\n\n\n\n",summary_results
        #return JsonResponse({"summary":json.dumps(summary_results)})
